...
program.action(async () => {
  ...
  try {
    ...
    const templatePath = path.join(__dirname, "..", "templates", templateType as string);
    ...
    const selectedChainId = chainIds[network as string];
    ...
    if (await fs.pathExists(routePath)) {
      ...
      routeContent = routeContent.replace(/{{NETWORK}}/g, network as string);
